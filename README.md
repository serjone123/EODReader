...
- `Electrode.LayoutForm.pas` — форма разметки на статическом изображении
  ...
  |прогноз. амплитуда| канала, цвет = цвет канала); не компилируется в
  песочнице Free Pascal (использует FMX).
...
Договорённость — перейти на проверку по реальным записям; на момент этой
записи реальные тестовые данные не собраны. Подсистему пока нельзя считать
проверенной на реальных данных.
