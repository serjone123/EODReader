...
- записи формируются последовательным проходом исходного WAV, поэтому `Position` не может уменьшаться от записи к записи;
- `Position` должны идти в порядке возрастания; это является следствием линейного алгоритма детектирования, а не требованием выполнять полную проверку сортировки при каждом открытии файла;
...
### Перекрытие окон соседних пиков

Окна waveform соседних пиков могут пересекаться. Это допустимо и не считается ошибкой формата:
...
Каждый пик хранит собственное окно waveform независимо от других пиков. Пересекающиеся исходные отсчёты могут присутствовать в обоих окнах.

При построении envelope cache данные пиков также агрегируются независимо. Cache не выполняет дедупликацию или объединение перекрывающихся окон.

Это корректно для визуализации и анализа отдельных событий. Однако при будущих расчётах общей энергии, длительности или статистики по уникальным временным отсчётам необходимо явно определить политику обработки перекрытий, чтобы не учитывать одну и ту же область сигнала дважды.
