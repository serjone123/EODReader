...
type
  ...
  TEodVideoExportThread = class(TEodBackgroundThread)
  private
...
procedure TEodVideoExportThread.DoFinished;
begin
  if Assigned(FOnFinished) then
    FOnFinished(Self, FCanceled, FErrorText, FRequest.OutputFileName);
end;

procedure TEodVideoExportThread.RunTask;
var
...
begin
  ...
  begin
    ...
    try
      ...
      try
        ...
        { При отмене RenderFrames возвращает False и не бросает
          исключение — превращаем это в EAbort. }
        CheckCancel;

        if Writer.FinishAndWait <> 0 then
          raise Exception.Create('ffmpeg (рендер полосы) завершился с ошибкой');
      finally
        Writer.Free;
      end;
    finally
  ...
  { Текст ошибки ffmpeg превращается в исключение; базовый класс
    запишет его в FErrorText и выставит FCanceled, если была отмена. }
  if not RunOverlayComposite(
    FRequest.FfmpegPath,
    ...
    StageBError) then
    raise Exception.Create(StageBError);
